...
// Métodos de pago que el propio usuario registró sobre el catálogo (su
// tarjeta, su cuenta, etc.). Viven en `metodos_pago_usuario`, con RLS que
// solo permite a cada usuario leer y modificar sus propios registros
...
export async function eliminarMetodoPagoUsuario(id: string) {
  const { error } = await supabase.from('metodos_pago_usuario').delete().eq('id', id)
  if (error) throw error
}
